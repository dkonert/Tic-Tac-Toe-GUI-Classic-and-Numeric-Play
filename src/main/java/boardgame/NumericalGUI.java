...
public class NumericalGUI extends JPanel {
    ...
    private NumericalTicTacToe game;
    ...
    public NumericalGUI(GUI gameFrame){
        ...
        setGameController(new NumericalTicTacToe());


        // make a new label to store messages
        messageLabel = new JLabel("Welcome to Numerical Tic Tac Toe");
    ...
    private void enterNumber(ActionEvent e){
        ...
        String input;
        if (game.getTurn() == 1) {
            input = JOptionPane.showInputDialog("Enter an odd number from 1 - 9");
        }else {
            input = JOptionPane.showInputDialog("Enter an even number from 2- 8");
        }
        PositionAwareButton clicked = ((PositionAwareButton)(e.getSource()));
        if(game.takeTurn(clicked.getAcross()-1, clicked.getDown()-1,input)){
